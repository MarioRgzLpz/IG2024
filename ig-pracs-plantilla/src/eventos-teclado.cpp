...
void FGE_PulsarLevantarTecla( GLFWwindow* window, int key, int scancode, int action, int mods )
{
   ...
   if ( glfwGetKey( window, GLFW_KEY_L) == GLFW_PRESS )
   {
      ...
      // Obtener la colección de fuentes de luz de la escena
      ColFuentesLuz *coleccion = escena->colFuentes();

      // Forzar revisualizar la escena si se ha modificado la colección de fuentes de luz
      apl->revisualizar_escena = ProcesaTeclaFuenteLuz(coleccion, key);
